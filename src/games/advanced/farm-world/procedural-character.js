/**
 * Procedural doll character — ported from fairy-worlds
 * (src/three/proceduralCharacter.js) as Farm World's second playable explorer.
 *
 * Everything is three.js primitives (no textures, no image assets — respects
 * the Advanced Mode no-image rule). The factory returns the same handle shape
 * as the source: { root, setColor, setVariant, setAccessory,
 * setAccessoryColor, getState, applyState }, and the customizer UI is driven
 * by PROCEDURAL_CUSTOMIZATION_SCHEMA so the options always match the builder.
 *
 * Port deltas from fairy-worlds (kept deliberately minimal):
 *  - setVariant disposes the replaced group's geometries so outfit swaps
 *    don't accumulate detached GPU buffers over a kiosk session.
 */
...
function makeHairVariant(id, material) {
  ...
  if (id === 'bob') {
    ...
    // per-variant clone (unlike every other variant's shared material) —
    // flagged so disposeVariantGroup releases it when the hairstyle changes
    afroMat.userData.perVariantClone = true
...
/** Dispose the geometries of a detached variant group. Materials are shared
    with the live character — except per-variant clones (afro), which are
    flagged and released along with the group. */
function disposeVariantGroup(group) {
  group.traverse(obj => {
    if (obj.geometry) obj.geometry.dispose()
    if (obj.material?.userData?.perVariantClone) obj.material.dispose()
  })
}
...
export function createProceduralCharacter(initialState = {}) {
  ...
  // for the walk gait. Arms/legs AND limb-riding outfit parts (sleeves, socks,
  // trouser legs, cuffs) are parented here so the whole limb swings as one.
  // (fairy-worlds never animated this character; the rig is Farm World's.)
  const limbs = {
  ...
  // host hook: world.js re-applies shadow flags after a variant rebuild
  let onRebuild = null
  function setOnRebuild(fn) { onRebuild = fn }

  return {
    root,
    limbs, // shoulder/hip pivot groups — the host swings these for the gait
    setColor,
    setVariant,
    setAccessory,
    setAccessoryColor,
...
// Factory-default state — used by the customizer's "Reset" (via applyState).
export const PROCEDURAL_DEFAULTS = DEFAULTS
...
export const PROCEDURAL_CUSTOMIZATION_SCHEMA = {
  ...
  variants: [
    { id: 'hair', label: 'Hair', options: ['bob', 'long', 'pigtails', 'bun', 'afro', 'fade', 'twinbuns', 'braids', 'mohawk'] },
    { id: 'shirt', label: 'Shirt', options: ['tee', 'tank', 'crop', 'long', 'overalls'] },
