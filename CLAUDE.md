...
**Farm World** (`adv-farm-world`) — Coastal World-inspired 3D farm explorer ([src/games/advanced/farm-world/](src/games/advanced/farm-world/)). Walk a mascot character around two low-poly procedural islands (three.js — second exception to the no-image-assets rule: WebGL canvas, but zero textures/image files) and visit 12 stations — 7 on a ring around the main island, 5 along a winding trail on a second island reached by a wooden bridge. Each station shows a lesson → 2 multiple-choice questions → sourced fact, then visually restores that corner of the farm. Question pools reuse the Trivia Blitz rounds (all 12 rounds now have a station); facts reuse existing IMPACT_MESSAGES ([src/data/content/advanced/farm-world.js](src/data/content/advanced/farm-world.js)). The in-world HUD is deliberately Coastal-style (white rounded cards, identical in both themes); the Soil Health meter expands on tap into a per-station restore checklist; the world screen `.adv-fw-world` is excluded from the advanced-mode position clamp in advanced.css. Movement is clamped to a union of regions (two island circles + a bridge capsule) in world.js; standable raised surfaces register in `walkables` for the ground-height raycast.

Four **NPCs** (procedural dolls) wander patrol routes across both islands — walk up and press E / tap Talk for short sourced tips. Dialogue is an in-scene speech bubble (no modal blur): the camera zooms to frame the pair, the NPC pauses and faces you, and only the line text floats/fades between lines. Their looks, dialogue lines, and route keys are plain data in [src/data/content/advanced/farm-world-npcs.js](src/data/content/advanced/farm-world-npcs.js); design a look in the in-game customizer and run `fwExportLook()` in the browser console to dump the doll-state JSON for pasting into an NPC's `look`.
...
- Controls: on-screen virtual joystick (kiosk) + WASD/arrows (laptops); E/Enter/Space or the Visit button to interact. Drag the scene to orbit the camera, wheel/two-finger-pinch to zoom, ⊙ topbar button resets the view. ✦ toggles between the two playable explorers — the "doll" ported from fairy-worlds (default; [procedural-character.js](src/games/advanced/farm-world/procedural-character.js)) and the farmer mascot. ☺ opens the player customizer, which follows the active character: farmer rows (body/hat/shoes/backpack swatches) or the doll's schema-driven rows (native color pickers, hair/shirt/bottom/wings variants, 14 accessories with per-accessory colors). The whole look (incl. active character + doll state) persists in sessionStorage `sdshc-fw-look`. The theme-toggle slot in the world topbar is a day/night switch for the 3D scene (default day) — the app theme toggle only appears on the Farm World intro.
