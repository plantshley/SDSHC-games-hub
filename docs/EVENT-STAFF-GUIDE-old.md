...
If you *do* want students naming their own groups, the school prefix is still recommended. Tell them to include the school, and keep the admin panel open on a second device to approve each group as it registers.
...
3. **Students** pick the school every time, and type their own name in the player field.
4. **After:** nothing.
...
- **A row can be a group, not just one student** (common in [Path B](#path-b--class-teams-one-classroom-groups-compete)): the group picks its team from the dropdown and puts the group name or a representative's name in the name field. Either way, the points go to the **team**, so what they type as the name doesn't affect the score — it's just a label on the row.
...
- **One device serves up to 4 students (or groups) at a time.** Size your groups to that. 
...
3. **"The points go to your team, not to you."** So the name field is just a label — one student can put their own name; a group can put the group's name or pick one person to type. Nothing about the score depends on it.

If groups are sharing a slot ([Path B](#path-b--class-teams-one-classroom-groups-compete)), add one line: **"Huddle up, pick your group's team, and take turns on the questions."**
