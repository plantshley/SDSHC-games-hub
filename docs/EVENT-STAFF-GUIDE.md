...
**Printable cheat sheet:** [event-cheat-sheet.html](event-cheat-sheet.html) — open it in a browser and print.
...
3. [Choose your event shape](#choose-your-event-shape)
...
12. [Running the room](#running-the-room--practical-playbook)
...
> **Contact:** ashley@sdsoil.org · 605-251-4364 (text, not call)
> **Lead time:** a few days ahead or earlier.
...
Ashley creates the event — starting it now, or scheduling it to **open automatically** at the event time — and can pre-create the teams. Once it's open, **every prepped device joins it automatically** the moment it enters Advanced Mode (see [device prep](#before-the-event--device-prep)).
...
- **Advanced Mode** — sleek, modern games for high school and college. **This is the mode with teams, events, and the leaderboard.** Everything in this guide is about Advanced Mode.
...
| ⚡︎ Trivia Blitz | Timed multiple-choice rounds |
...
(tap the 𖧧 button to play a special secret game 👀)
...
| **A "team" is** | Each school | Each small group of students | The whole school, with individual students or groups as "players" |
| **A "player" is** | An individual student (use their name) or a group of students (use the group/team name, or a representative's name) | An individual student (use their name) or a group of students (use the group/team name, or a representative's name) | An individual student (use their name) or a group of students (use the group/team name, or a representative's name) |
| **Who creates teams** | You (recommended beforehand) or Students | You or Students | You (recommended beforehand) |
| **How many teams** | 2–10+ | 4–8 | 1 |
| **Approve for event** | Yes, each school | Yes, each group | Yes |
| **Approve statewide** | Yes, each school | **No** | Yes |
...
If you choose to pre-create and approve the teams rather than have the students invent their own team names live, the Team dropdown on each game's intro screen will list these teams for students to pick from. If not, each group can create a name by hand, and those names arrive as *pending*, and **the event leaderboard stays empty until you approve each one**. 

If you pre-create teams, here are some recommendations: 

1. **Name the groups with the school as a prefix:** `Lincoln – Table 1`, `Lincoln – Table 2`, … **Not** bare `Table 1`. See the warning below.
...
If you *do* want students naming their own groups, the school prefix is still recommended. Tell them to include the school, and you keep the admin panel open on a second device to approve each group as it registers.
...
3. **Students** pick the school every time, and type their own (or group's) name in the player field.
4. **After:** nothing.
...
**Do all of this at home, on good Wi-Fi, on the exact device you're bringing.** If using a computer or device at the event/school/classroom itself, you can **skip steps 1 and 2** and just use the website link. However, this runs of the risk of the URL being blocked by school Wi-Fi or other unforseen issues. **Step 5** can also be done separately on a different device prior to the event. 
...
3. Enter the **admin password**. (Ask Ashley — it's one shared password.)
...
> 🔒 **Stop the browser from remembering the admin password (one time, per device).** The first time you sign in, Edge or Chrome will pop up **"Save password?"** — click **Never**. If it already saved it, delete it so it can't auto-fill for whoever opens the panel next:
> - **Edge:** Settings → **Profiles → Passwords** → find the site → **⋯ → Delete**. Then turn **"Offer to save passwords"** off.
> - **Chrome:** Settings → **Autofill and passwords → Google Password Manager** → the entry → **Delete**. Then turn **"Offer to save passwords"** off.
>
...
3. See the game grid with **no** play prompt? No event is open yet. If you **scheduled** yours for later, that's expected — it'll open itself when the start time arrives. To open it right now instead, go to **Admin → Events**, find yours, tap **"Open now"**, then reopen Advanced Mode.
...
Perfectly supported, and maybe useful for a large class. Also, note that each game supports multiplayer (or group) up to four. 
...
1. Type an **event name**. Students see this as their leaderboard tab, so name it something identifiable, like `Brookings Middle School Visit`, not `event3`.
...
| **Schedule for** | Event is created with status `scheduled`. It stays dormant until its start time, then **opens itself automatically** — on every device, no sign-in needed. It just isn't joinable (or in the "Set kiosk to" dropdown) *before* that time. |

> **Scheduling opens the event for you** at the time you set. Two things to know: (1) each device opens it using **its own clock**, so a badly-set device clock shifts when it flips open; (2) to open a scheduled event *early*, tap **"Open now"** on its row. If you'd rather not think about timing, "Start now" on the morning of is simplest.

### Managing events

The **Events** section lists everything with a status badge that's **derived from the clock**, not a field someone flips — a scheduled event whose time has passed shows `open`, and an event past its end time shows `ended (auto)` even if no one tapped End.

| Status | Meaning | Actions |
|---|---|---|
| `scheduled` | Created for later; opens itself at its start time | **Open now** (opens early), **Ends** time, Delete |
| `open` | Running — accepts scores | **End**, **Ends** time, Delete |
| `ended` | Finished (by hand or aged out), still on the leaderboard | **Reopen**, Delete |

- **Open now** opens a scheduled event *ahead* of its start time. You don't need it at the scheduled time itself — it opens on its own then.
- **End** closes the event now. If it was active on your device, that's cleared. Other devices notice and stand down next time they're online.
- **Reopen** un-ends it (ended by accident, or a multi-day event) and pushes its end time back out so it doesn't immediately re-close.
- **Ends** — each running/scheduled row has an editable end date/time. This is when the event closes itself.
- **Delete** ⚠️ **permanently destroys the event *and every score tagged to it*.** No undo. When an event is simply over, use **End**.

> **Events close themselves so nothing stays joinable forever.** Every event auto-ends **24 hours after it starts** by default. For a longer or multi-day event, edit the **"Ends"** date/time on its row to push that out. You can still **End** it early at any time, and a row that closed itself shows `ended (auto)`.
...
- **A "Player" can be a group, not just one student**: the group picks its team from the dropdown and puts the group name or a representative's name in the name field. Either way, the points go to the **team**, so what they type as the name doesn't affect the score — it's just a label on the row.
...
Teams are created via the team roster screen, and creates them as **pending**. Teams can be viewed from three places:
...
### Naming — reminder for students

Points only add up if the team name matches. Capitalization and extra spaces are ignored; spelling is not. `Pierre HS` and `Pierre High` are two different teams.

And remember [team names are global](#path-b--class-teams-one-classroom-groups-compete), so a generic name like `Table 1` or `Red Team` could collide with some other identical name. (e.g. why prefixing with the school is recommended).
...
|---|---|---|
| **Good, open Wi-Fi** | Play via installed app shortcut (recommended) or browser with website link. | Saved to the cloud immediately. Leaderboard is live. |
| **No Wi-Fi at all** | Must play via installed app shortcut. | Queued on the device. Sync automatically when you get home and open the app online. |
...
1. If you **ran the event offline,** get the device on trusted Wi-Fi and **open the app**. **Leave it open a minute or two** — this is when queued scores flush to the cloud. Watch the leaderboard counts climb. Do this for **every device** you ran.
> *(feel free to save the remaining steps for Ashley)*
...
3. **Approve statewide** the teams that should reach the All-Time board (Paths A and C), if not done already.
...
6. Tell Ashley how it went — especially if you encountered any bugs or have any feedback from students. 
...
| **Small tablet / large phone** | ⚠️ Great for **admin access**, but limited with gameplay. **Game screens won't load below ~600px wide** — they show a "screen too narrow, switch to landscape orientation" message. Field Guide needs ~900px. **Use landscape.** Menus, roster, and admin work at any size. |
...
- Players on **one device can be on different teams** — set up competition on a single machine.
...
- **Show the leaderboard between games** Tap the trophy and put it on the big screen. Could be great motivation seeing a team's placement change. 
- Some games end with a short conservation or educational takeaway. Let it sit a beat before the next game or question.

### Choosing an event shape for the setting
...
| General event, walk-up traffic | **Casual Play** | No name-entry friction; nobody's tracking a stranger's points |
...
| **Scheduled event isn't open at its start time** | Its start hasn't actually passed on the device's clock, or the device clock is wrong | Check the device date/time; or just tap **"Open now"** to open it immediately |
| **Event closed itself / shows `ended (auto)`** | It aged out — events auto-end 24 h after they start | **Reopen** it, then push its **"Ends"** time out for the longer window |
...
| **"Couldn't save score — tell an organizer"** | A genuine save failure | Note the team + game and inform Ashley. Scores usually still queue; check after syncing. |
| **Game says "screen too narrow"** | Narrower than ~600px (Field Guide: ~900px) | Rotate to landscape, or use a bigger device |
| **App won't load at the school** | Site blocked and device never prepped | Nothing on-site. Use a mobile hotspot or prep at home next time. |
...
| Scheduled events | **Open themselves** at their start time (device clock). "Open now" opens one early |
| Open events | **Auto-end 24 h after they start.** Push the "Ends" time out for longer; "End" closes early |
